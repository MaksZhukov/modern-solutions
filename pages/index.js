...
const help1cYouTabs = ['Бухгалтерам', 'Малому бизнесу', 'Интернет магазинам', 'Торговым компаниям'];
            ...
            <Container>
                ...
                <Tabs className={styles['helps-1c']}>
                    <TabList className={styles['helps-1c__tabs']}>
                        {help1cYouTabs.map((item) => (
                            <Tab
                                ...
                                selectedClassName={styles['helps-1c__tab_selected']}
                                key={item}>
                                {item}
                            </Tab>
                        ))}
                    ...
                    {help1cYouTabsContent.map((item) => (
                        <TabPanel key={item.title} className={styles['helps-1c__pane']}>
                            <img className={styles['helps-1c__img']} src={item.url} alt={item.title}></img>
                            <div className={styles['helps-1c__content']}>
                                <h2 className={styles['helps-1c__title']}>{item.title}</h2>
                                <ul>
                ...
                <h2 className={styles['title']}>Услуги, которые мы оказываем в рамках договора</h2>
                <div className={styles['additional-services']}>
                    {additionalServices.map((item) => (
                        <div key={item.name} className={styles['additional-services__item']}>
                            <img className={styles['additional-services__img']} src={item.url} alt={item.name} />
                            <h3 className={styles['additional-services__title']}>{item.name}</h3>
                            <p>{item.desc}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className={styles['stages-of-cooperation']}>
                    <div className={styles['stages-of-cooperation__item']}>
                        <span className={styles['stages-of-cooperation__num']}>1</span>
                        <div className={styles['stages-of-cooperation__text']}>
                            Вы звоните нам по телефону{' '}
                            <a className={styles['stages-of-cooperation__link']} href='tel:+375257402263'>
                                +375 (25) 740-22-63
                            </a>{' '}
                            или пишите на почту{' '}
                            <a className={styles['stages-of-cooperation__link']} href='mailto:it-cooperation@yandex.by'>
                    ...
                    <div className={styles['stages-of-cooperation__item']}>
                        <span className={styles['stages-of-cooperation__num']}>2</span>
                        <div className={styles['stages-of-cooperation__text']}>
                            Мы проведем комплексный анализ вашей деятельности и выявим необходимые доработки.
                        </div>
                    </div>
                    <div className={styles['stages-of-cooperation__item']}>
                        <span className={styles['stages-of-cooperation__num']}>3</span>
                        <div className={styles['stages-of-cooperation__text']}>
                            Разработаем техдокументацию, зафиксируем все важные для вас параметры.
                        </div>
                    </div>
                    <div className={styles['stages-of-cooperation__item']}>
                        <span className={styles['stages-of-cooperation__num']}>4</span>
                        <div className={styles['stages-of-cooperation__text']}>
                            Модифицируем программу и тестируем её.
                        </div>
                    </div>
                    <div className={styles['stages-of-cooperation__item']}>
                        <span className={styles['stages-of-cooperation__num']}>5</span>
                        <div className={styles['stages-of-cooperation__text']}>
                            Встраиваем новый функционал в рабочую базу. Проводим обучение сотрудников.
                        </div>
                    </div>
                </div>
